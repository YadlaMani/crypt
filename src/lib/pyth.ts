...
export class PythPriceService {
   ...
   */
  async getTokenPrice(tokenSymbol: keyof typeof PRICE_FEED_IDS): Promise<number> {
    try {
   ...
   */
  async convertUsdToNativeToken(usdAmount: number, chainId: string): Promise<{
    nativeAmount: number;
    tokenSymbol: string;
    price: number;
  }> {
    try {
      const tokenSymbol = CHAIN_NATIVE_TOKENS[chainId as keyof typeof CHAIN_NATIVE_TOKENS];
      ...
      console.error(`Error converting USD to native token for chain ${chainId}:`, error);
      throw error;
    }
  }
   ...
   */
  async getMultipleTokenPrices(tokens: Array<keyof typeof PRICE_FEED_IDS>): Promise<Record<string, number>> {
    try {
      const feedIds = tokens.map(token => PRICE_FEED_IDS[token]).filter(Boolean);
   ...
   */
  async getAllChainConversionRates(usdAmount: number): Promise<Record<string, {
    nativeAmount: number;
    tokenSymbol: string;
    price: number;
  }>> {
...
export function formatNativeAmount(amount: number, tokenSymbol: string): string {
